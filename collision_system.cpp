...
bool check_collision_explosion(int ent_id, int explosion_id) { // Returns true if entity's shape component is within an explosion's shape component
...
int return_id_colliding_explosion(int ent_id) { // returns of ID of explosion entity that is colliding with entity

	std::vector<int> ids{};

	int result{ 0 };

	ids = ECS_obj.return_ids_with_name("explosion");

	for (int index = 0; index < std::size(ids); index++) {

		if (check_collision_explosion(ent_id, ids[index])) {
...
void explosion_health_response(int ent_id) { // if entity is colliding with explosion, subtract 1.0f health

	float current_health{ ECS_obj.get_entity_components(ent_id).m_health };

	float damage{ 1.0f };

	if (return_id_colliding_explosion(ent_id) != 0) { // if returned ID is 0, no collision is occuring

		ECS_obj.set_health(ent_id, current_health - damage);

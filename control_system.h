...
void thrust_check(int ID) { // checks if KEY_UP/KEY_DOWN is pressed and updates force vector in ECS
	if (ECS_obj.ECS_map[ID].m_name == "ship") { // maybe add "movable" component later and check for that
		// initializing local vars
		float angle{ ECS_obj.ECS_map[ID].m_angle };
		Vector2 force{ ECS_obj.ECS_map[ID].m_force };
		Vector2 thrust{ cos(ECS_obj.ECS_map[ID].m_angle) * 10.0f, sin(ECS_obj.ECS_map[ID].m_angle) * 10.0f };
		...
		ECS_obj.update_entity_components(
			...
			ECS_obj.ECS_map[ID].m_mass,
			ECS_obj.ECS_map[ID].m_color,
			ECS_obj.ECS_map[ID].m_position, ECS_obj.ECS_map[ID].m_velocity, ECS_obj.ECS_map[ID].m_acceleration, force,
			ECS_obj.ECS_map[ID].m_angle, ECS_obj.ECS_map[ID].m_angvel, ECS_obj.ECS_map[ID].m_angacc, torque,
			ECS_obj.ECS_map[ID].m_shape,
...
void missile_control(int ID) { // applies linear force until velocity relative to target reaches a certain value
	if (ECS_obj.ECS_map[ID].m_name == "missile") { // maybe add "movable" component later and check for that
		// initializing local vars
		float angle{ ECS_obj.ECS_map[ID].m_angle };
		Vector2 force{ ECS_obj.ECS_map[ID].m_force };
		Vector2 thrust{ cos(ECS_obj.ECS_map[ID].m_angle) * 30.0f, sin(ECS_obj.ECS_map[ID].m_angle) * 30.0f };

		float torque{ ECS_obj.ECS_map[ID].m_torque };
		float turn_force{ 1500.0f };
		
		if (abs(ECS_obj.ECS_map[ID].m_angvel) <= 0.1f) force = thrust; // engines won't push if turning fast
